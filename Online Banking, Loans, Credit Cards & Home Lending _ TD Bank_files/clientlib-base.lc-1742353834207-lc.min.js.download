...
(function() {
    ...
    var properties = {
        ...
        /**
         * An array of alternative screen widths (in pixels).
         * Used to replace a {.screenwidths} variable in the src property with an optimal screenwidth if a URI template is provided.
         *
         * @memberof Image
         * @type {Number[]}
         * @default []
		...
		/**
		*As part of CMNP-1981-devicePixelRatio is now fixed to 1 for TD
    ...
    function Image(configVar) {
        ...
        function loadImage() {
            ...
            var hasScreenWidths = (that._properties.screenwidths && that._properties.screenwidths.length > 0);
            var replacement;
            if (Object.keys(smartCrops).length > 0) {
                var optimalWidth = getOptimalWidth(Object.keys(smartCrops));
                replacement = smartCrops[optimalWidth];
            } else if (hasScreenWidths && hasWidths) {
                replacement = (that._properties.dmimage ? "" : ".") + getOptimalScreenWidth(that._properties.widths, that._properties.screenwidths);
            } else {
...
// Default Adobe Code Start with 2.15.2
//            var optimalWidth = containerWidth * devicePixelRatio;
// Default Adobe Code End with 2.15.2

//            CMNP-1981 removing device pixel ratio
        ...
        // e.g widths [768, 1024, 1200, 0]
        // e.g screenwidts [226, 291, 320, 580]
         function getOptimalScreenWidth(widths, screenWidths) {
			var url = that._properties.src;
			if (url.indexOf('bannerimage') > -1) {
			var len = widths.length;
			var optimalWidth = window.innerWidth;
			let indexByValue, key;
			let tempArray = screenWidths.slice(); // create a copy of screenWidths array with new tempArray eg: [768, 1024, 1200, 0]
			tempArray = tempArray.sort(function (a, b) { return a - b }); //Sort tempArray eg: [0,768,1024,1200]

			// loop to get the matching key from tempArray based on user screen width
			for (var i = 0; i < len; i++) {
				if (optimalWidth >= tempArray[i]) {
					key = i;
				}
			}
			//get the index value from screenWidths array based on tempArray key
			indexByValue = screenWidths.indexOf(tempArray[key]);
			return widths[indexByValue].toString();
		} else {
			var len = widths.length;
			var key = len-1; //default value set for last item since it is for mobile break point
			var optimalWidth = window.innerWidth;

			for(var i=0; i < len -1; i++){ //run the loop till last -1 items since if condtion will be always true for last item( mobile breakpoint)
				if(optimalWidth >= screenWidths[i]){
					key = i;
				}
			}
			return widths[key].toString();
        ...
        if (configVar && configVar.element) {
            init(configVar);
        }
    }
